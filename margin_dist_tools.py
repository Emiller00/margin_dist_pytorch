...
def get_margin_distribution(model,X,y,margin_length=4):
    ...
    for name, module in model.named_modules():
       if isinstance(module, (nn.Linear,  nn.Flatten,nn.MaxPool2d,nn.Conv2d)):  # Modify this condition based on the types of layers you want
    #        print(module)
            intermediate_inputs.append(getattr(module, "_input"))

    for name, module in model.named_modules():

       #if isinstance(module, (nn.Linear,  nn.Flatten,nn.MaxPool2d,nn.ReLU)):  # Modify this condition based on the types of layers you want
       if isinstance(module, ( nn.Flatten,nn.MaxPool2d,nn.ReLU,nn.Conv2d)):  # Modify this condition based on the types of layers you want
            #print(module)
            intermediate_outputs.append(getattr(module, "_output").view(batch_size,-1).detach().numpy())

    intermediate_outputs = intermediate_outputs[:margin_length]

    # We are only going over the first 4 layers so we don't need this
    # The issue is that tensorflow and pytorch both use 
    #intermediate_outputs.append(output.view(batch_size,-1).detach().numpy())

    #num_ints = len(intermediate_inputs)
...
def get_normalized_margin(md,int_out):
